...
from all_sky_cloud_detection.celestial_objects import moon_coordinates, sun_coordinates
...
def process_image(path, cam, show_plot=False, save_plot=False):
    """
    Process an image from an allsky camera to get a cloudiness value.
    ...
    moon_altitude: 'float'
        Altitude of the moon in the image [radians]
    sun_altitude: 'float'
        Altitude of the sun in the image [radians]
    """
    ...
    sun_altitude = sun_coordinates(time, cam)
    ...
    return cloudiness, time, mean_brightness, moon_altitude, sun_altitude
